...
사용자 ID 기반 QR 코드를 발급하여 소유자가 직접 물건에 부착 가능

습득자가 QR 코드를 스캔하여 분실물 정보를 입력하면 분실자에게 직접 전달되는 시스템 제공
...
사용자 ID 기반 QR 코드 발급 및 다운로드

QR 코드 스캔을 통한 습득물 데이터 입력 및 분실자에게 이메일 전송 및 연락처 제공

분실물 지도 기반 검색으로 주변 분실물 센터 및 등록된 분실물 확인

습득물 등록 시 지도에 마커 자동 등록

🎯 페르소나

🙋‍♀️ 남숙희 (25세, 사회초년생)

출퇴근 시 바쁘며 소지품이 많아 물건을 자주 잃어버림

스마트폰 등 디지털 기기와 지갑을 주로 분실함

니즈:

빠르고 효율적으로 잃어버린 물건을 찾을 수 있는 서비스

습득자가 손쉽게 주인을 찾아 연락 가능한 시스템

Pain Point:

물건을 분실했을 때 어디서부터 찾아야 할지 막막함

습득자가 연락할 방법이 없어 회수 불가능한 경우 많음
...
Java (JDK 1.8), JavaScript, Python

개발 도구

STS, VSCode, Colab, Jupyter

프레임워크 및 라이브러리

Spring, Bootstrap, MyBatis, ZXing, Selenium, Pandas, Numpy, Matplotlib
...
경찰청 API, Naver Works Mail API, Kakao Map API
...
문제: 경찰청 API 데이터를 한 번에 받아오면 처리 속도가 느림

해결: 데이터를 3회에 나눠 받아와 즉시 테이블을 생성해 처리 속도를 개선

2️⃣ 습득물 보관장소 선택 오류

문제: 보관장소 미선택 시 오류 발생

해결: 미선택 여부를 구분하기 위해 기본값을 위도 0, 경도 180으로 설정

3️⃣ QR 코드 스캔 시 경로 문제

문제: QR 코드 접속 시 경로에 qrScan 추가되는 오류 발생

해결: QR 코드 기능에만 별도 경로 매핑 및 무관한 기능은 redirect로 정상 경로로 이동
